package com.example.smartmedicalsystem.ui.theme.screens.screens.Medicine.screen// package com.example.smartmedicalsystem.ui.theme.screens.screens.Medicine.screen
//
...
fun AddMedicineScreen(navController: NavController) {
    ...
    // BUG FIXED #5 – isLoading was set to `false` immediately after calling
    // uploadMedicine() because the upload runs on a coroutine (async).
    // The fix: pass an onDone callback into the ViewModel so isLoading is
    // only cleared when the coroutine actually finishes.
    var isLoading     by remember { mutableStateOf(false) }
    var errorMessage  by remember { mutableStateOf<String?>(null) }
            ...
            Box(
                ...
                Column(
                    ...
                    OutlinedTextField(
                        ...
                        onValueChange = { frequency = it },
                        label         = { Text("Frequency (e.g. 08:00 / 14:00)") },
                        placeholder   = { Text("e.g. 08:00 / 14:00") },
                        modifier      = Modifier.fillMaxWidth(),
                        shape         = RoundedCornerShape(12.dp)
                    )
                    ...
                    Button(
                        onClick = {
                            if (name.isBlank() || dosage.isBlank()) {
                                errorMessage = "Medicine name and dosage are required"
                                return@Button
                            }
                            errorMessage = null
                            isLoading    = true   // ← set BEFORE launching
                            ...
                            medicineViewModel.uploadMedicine(
                                ...
                                onDone      = { isLoading = false }  // ← cleared AFTER coroutine finishes
                            )
                            // ✅ Do NOT set isLoading = false here – the upload is async
                        },
                        modifier = Modifier
                            .fillMaxWidth()
                            .height(50.dp),
